...
parismap = np.array(Image.open("data/paris-48.806-2.23--48.916-2.48.jpg"))
poi_names = ["furniture_store", "laundry", "bakery", "cafe",
             "home_goods_store", "clothing_store", "atm",
             "lodging", "night_club", "convenience_store",
             "restaurant", "bar"]
...
class Histogramme(Density):
    def __init__(self,steps=10):
        ...
        self.steps = steps
        self.stepsize = np.array([(xmax-xmin)/self.steps,
                                  (ymax-ymin)/self.steps])
        print("stepsize = ", self.stepsize)
    def fit(self,x):
        #A compléter : apprend l'histogramme de la densité sur x
        self.hist, _, _ = np.histogram2d(x[:, 0], x[:, 1], bins=(self.steps, self.steps), density=True)
    def predict(self,x):
        #A compléter : retourne la densité associée à chaque point de x
        x = np.floor((x-np.array([xmin, ymin]))/self.stepsize).astype(int)
        y, x = zip(*[tuple(i) for i in x.tolist()])
        return self.hist[y, x]
...
def show_poi(poi_liste):
    """
    Afficher la map, et les points d'interet passé en parametre
...
pois = {i: poi_name for i, poi_name in enumerate(poi_names)}
...
geo_mat, notes = load_poi("laundry")
...
show_density(model, geo_mat, steps, True)
